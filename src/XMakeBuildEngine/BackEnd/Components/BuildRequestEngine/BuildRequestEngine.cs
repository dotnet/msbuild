    ...
    internal class BuildRequestEngine : IBuildRequestEngine, IBuildComponent
    {
        ...
        private void TraceEngine(string format, params object[] stuff)
        {
            if (_debugDumpState)
            {
                lock (this)
                {
                    using (StreamWriter file = FileUtilities.OpenFile(String.Format(CultureInfo.CurrentCulture, Path.Combine(_debugDumpPath, @"EngineTrace_{0}.txt"), Process.GetCurrentProcess().Id), append: true))
                    {
