        ...
        internal object Execute(object objectInstance, IPropertyProvider<P> properties, ExpanderOptions options, IElementLocation elementLocation)
        {
            ...
            try
            {
                ...
                for (int n = 0; n < _arguments.Length; n++)
                {
                    ...
                    if (argument is string argumentValue)
                    {
                        ...
                        if ((_receiverType == typeof(System.IO.File)
                                ...
                                || (_receiverType == typeof(System.IO.Path)
                                    && _methodMethodName.Equals("Exists", StringComparison.OrdinalIgnoreCase)))
                ...
                if (String.Equals("new", _methodMethodName, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        functionResult = LateBindExecute(null /* no previous exception */, BindingFlags.Public | BindingFlags.Instance, null /* no instance for a constructor */, args, true /* is constructor */, out invocationArguments, out boundMethod);
                    }
                }
                else
                {
                    ...
                    try
                    {
                        ...
                        _propertiesUseTracker.InputRecorder?.RecordPropertyFunctionInput(
                            _receiverType,
                            _methodMethodName,
                            objectInstance is not null,
                            args);
                    ...
                    if (!wellKnownFunctionSuccess)
                    {
                        ...
                        catch (MissingMethodException ex) when ((_bindingFlags & BindingFlags.InvokeMethod) == BindingFlags.InvokeMethod)
                        {
                            ...
                            functionResult = LateBindExecute(ex, _bindingFlags, objectInstance, args, false /* is not constructor */, out invocationArguments, out boundMethod);
                        }
                    }
                }

                if (_propertiesUseTracker.InputRecorder is { } inputRecorder)
                {
                    if (boundMethod is null
                        && PropertyFunctionEffects.RequiresBoundMethod(_receiverType))
                    {
                        boundMethod = TryResolvePublicMethodForRecording(
                            invocationArguments ?? args);
                    }
                    inputRecorder.RecordPropertyFunction(
                        _receiverType,
                        _methodMethodName,
                        objectInstance is not null,
                        boundMethod,
...
#pragma warning restore 618, 612
            ...
            out MethodInfo boundMethod)
        {
            ...
            invocationArguments = args;
            boundMethod = memberInfo as MethodInfo;
            return functionResult;
        }
        ...
        [UnconditionalSuppressMessage(
            "Trimming",
            "IL2080",
            Justification = "_bindingFlags is masked to AllowedBindingFlags, so GetMethods binds only the public property-function allowlist surface.")]
        private MethodInfo TryResolvePublicMethodForRecording(object[] arguments)
        {
            if ((_bindingFlags & BindingFlags.InvokeMethod) == 0
            ...
            try
            {
                MethodBase[] candidates = _receiverType.GetMethods(_bindingFlags)
