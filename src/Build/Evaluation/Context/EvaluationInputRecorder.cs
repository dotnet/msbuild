...
internal sealed class EvaluationInputRecorder
{
    ...
    /// </summary>
    internal bool IsRecording => !_frozen && _nonCacheable == NonCacheableReason.None;
    ...
    /// <summary>
    /// Records a project file whose content evaluation consumed with the supplied read-time timestamp and length.
    /// A file that changed since it was read, for example behind a cached <c>ProjectRootElement</c>, cannot be reused.
    /// </summary>
    internal void RecordProjectSource(string? fullPath, DateTime lastWriteTimeUtcWhenRead, long? lengthWhenRead)
    {
        ...
        try
        {
            ...
            if (lengthWhenRead is null)
            {
                MarkNonCacheable(NonCacheableReason.RecorderFailure, path);
                return;
            }
    ...
    private void RecordEnvironmentReferences(string text)
    {
        ...
        while (start >= 0)
        {
            ...
            int next = end + 1;
            while (escapedPercentCount > 0 && next < text.Length && text[next] == '%')
            {
                escapedPercentCount--;
                next++;
            }
    ...
    /// <summary>
    /// Records file and directory reads before invoking a property function so the observed file state cannot
    /// postdate the content consumed by evaluation.
    /// </summary>
    internal void RecordPropertyFunctionInput(
        Type receiverType,
        string member,
        bool isInstance,
        object?[] arguments)
    {
        if (!IsRecording || arguments.Length == 0 || arguments[0] is not string firstArgument)
        {
            return;
        }

        PropertyFunctionEffect effect = PropertyFunctionEffects.Classify(
            receiverType,
            member,
            isInstance,
            boundMethod: null,
            arguments);
        if (effect is PropertyFunctionEffect.ReadFile or PropertyFunctionEffect.ReadDirectory)
        {
            RecordPath(firstArgument);
        }
        else if (effect == PropertyFunctionEffect.ExpandEnvironment)
        {
        ...
        object?[]? invocationArguments = null)
    {
        ...
        object?[] effectiveArguments = invocationArguments ?? arguments;
        PropertyFunctionEffect effect = PropertyFunctionEffects.Classify(
            receiverType,
            member,
            isInstance,
        ...
        switch (effect)
        {
            ...
            case PropertyFunctionEffect.ReadEnvironment when firstArgument is not null:
                ...
                break;
            case PropertyFunctionEffect.ExpandEnvironment:
                break;
            case PropertyFunctionEffect.Registry:
                object?[] registryArguments = effectiveArguments;
    ...
    private static string Canonicalize(string path)
    {
        ...
        string fullPath = FileUtilities.NormalizePath(
            FileUtilities.MakeFullPathFromThreadWorkingDirectory(fixedPath)?.Value ?? fixedPath);
