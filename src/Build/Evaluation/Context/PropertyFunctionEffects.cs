...
internal static class PropertyFunctionEffects
{
    ...
    internal static bool RequiresBoundMethod(Type receiverType) =>
        string.Equals(
            receiverType.FullName,
            ToolLocationHelperTypeName,
            StringComparison.Ordinal);

    internal static PropertyFunctionEffect Classify(
        Type receiverType,
        string member,
        bool isInstance,
        ...
        object?[] arguments)
    {
        ...
        {
            if (!RequiresBoundMethod(receiverType))
            {
            ...
            return s_probingToolLocationHelperMembers.Contains(member)
                || boundMethod is null
                || HasCallerSuppliedPath(boundMethod, arguments)
                    ? PropertyFunctionEffect.Unsupported
                    : PropertyFunctionEffect.Pure;
        }
        ...
        {
            PropertyFunctionEffect.ReadEnvironment or PropertyFunctionEffect.ExpandEnvironment when arguments.Length != 1 => PropertyFunctionEffect.Unsupported,
            PropertyFunctionEffect.ReadDirectory when arguments.Length > 2 => PropertyFunctionEffect.Unsupported, // a SearchOption argument may recurse
            _ => effect,
        };
    }

    private static bool HasCallerSuppliedPath(MethodInfo method, object?[] arguments)
    {
        ...
        {
            if (!IsPathParameter(parameters[i].Name)
                || arguments[i] is null
                || arguments[i] is string { Length: 0 })
            {
                continue;
            }

            // Do not enumerate an arbitrary value just to classify it. A nonempty string,
            // or any non-null non-scalar value supplied to a semantic path parameter, fails closed.
            return true;
        }
    ...
    private static FrozenDictionary<Type, TypeEffects> CreateEffectsByType()
    {
        var byType = new Dictionary<Type, TypeEffects>
        {
            [typeof(Environment)] = new(PropertyFunctionEffect.Unsupported, new()
            {
                ...
                ["GetFolderPath"] = PropertyFunctionEffect.Unsupported,
            ...
            [typeof(Path)] = new(PropertyFunctionEffect.Pure, new()
            {
                ...
                ["GetRelativePath"] = PropertyFunctionEffect.Unsupported,
                ["GetTempPath"] = PropertyFunctionEffect.Unsupported,
