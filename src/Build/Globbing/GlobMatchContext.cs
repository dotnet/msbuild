...
internal struct GlobMatchContext(string stringToMatch)
{
    ...
    internal bool IsMatch(IMSBuildGlob glob)
    {
        switch (glob)
        {
            case MSBuildGlob parsedGlob:
                return parsedGlob.IsMatch(ref this);
            case CompositeGlob composite when glob.GetType() == typeof(CompositeGlob):
                return composite.IsMatch(ref this);
            case MSBuildGlobWithGaps gaps when glob.GetType() == typeof(MSBuildGlobWithGaps):
                return gaps.IsMatch(ref this);
            default:
                // Custom matchers may reimplement IMSBuildGlob or change ambient path resolution.
