                ...
                if (outerBuild.ProjectType == ProjectType.OuterBuild && outerBuild.ReferencingProjects.Count != 0)
                {
                    foreach (ProjectGraphNode innerBuild in outerBuild.ProjectReferences)
                    {
                        foreach (ProjectGraphNode outerBuildReferencingProject in outerBuild.ReferencingProjects)
                        {
                            ...
                            {
                                ProjectItemInstance existingInnerBuildEdge = graphBuilder.Edges[(outerBuildReferencingProject, innerBuild)];

                                if (existingInnerBuildEdge.ItemType.Equals(TransitiveReferenceItemName, StringComparison.OrdinalIgnoreCase))
                                {
                                    outerBuildReferencingProject.RemoveReference(innerBuild, graphBuilder.Edges);
                                }
                                else
                                {
                                    // An explicit reference to an inner build takes precedence over the edge generated
                                    // from the outer build. GraphEdges.AddOrUpdateEdge will preserve it and merge target metadata.
                                    Assumed.Equal(
                                        existingInnerBuildEdge.ItemType,
                                        ItemTypeNames.ProjectReference,
                                        StringComparison.OrdinalIgnoreCase,
                                        "Only direct or transitive references may reference inner builds that got generated by outer builds");
                                }
                            }
