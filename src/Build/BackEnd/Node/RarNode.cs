...
namespace Microsoft.Build.Execution
{
    public sealed class RarNode : INode
    {
        ...
        public NodeEngineShutdownReason Run(bool nodeReuse, bool lowPriority, out Exception shutdownException)
        {
            ...
            _rarTask = Task.Run(async () => /*_rarResult = */await controller.StartAsync(cts.Token).ConfigureAwait(false));
            // Exits in 15 mins (by default), if don't have any active connection
