        ...
        internal bool CreateNode(HandshakeOptions hostContext, INodePacketFactory factory, INodePacketHandler handler, TaskHostConfiguration configuration, Dictionary<string, string> taskHostParameters)
        {
            ...
            if (Handshake.IsHandshakeOptionEnabled(hostContext, HandshakeOptions.NET))
            {
                ...
                nodeContexts = GetNodes(
                    runtimeHostPath,
                    string.Format(commandLineArgsPlaceholder, Path.Combine(msbuildAssemblyPath, Constants.MSBuildAssemblyName), GetEnableNodeReuse(hostContext), ComponentHost.BuildParameters.LowPriority),
                    nodeId,
                    this,
                    handshake,
                    NodeContextCreated,
            ...
            nodeContexts = GetNodes(
                msbuildLocation,
                string.Format(commandLineArgsPlaceholder, string.Empty, GetEnableNodeReuse(hostContext), ComponentHost.BuildParameters.LowPriority),
                nodeId,
                this,
            ...
            bool GetEnableNodeReuse(HandshakeOptions hostContext)
            {
                bool isCLR2 = Handshake.IsHandshakeOptionEnabled(hostContext, HandshakeOptions.CLR2);
