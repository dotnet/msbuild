        ...
        private void ReportResultsToSubmission(BuildResult result)
        {
            lock (_syncLock)
            {
                ...
                if (_buildSubmissions.ContainsKey(result.SubmissionId))
                {
                     ...
                     * 
                     * If any other exception happened and logging is not completed, then go ahead and completed now since this is the last place to do it.
                     * Otherwise the submission would remain uncompleted and the build would hang.
                    */
