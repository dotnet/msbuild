...
using System;
using Microsoft.Build.NuGetFrameworks;

namespace Microsoft.Build.Evaluation;

// Vendored implementation: NuGet.Frameworks source is compiled directly into Microsoft.Build
// (see Utilities\NuGetFrameworks) and called without reflection or a secondary AppDomain. This is
// the Change Wave 18.9 default; NuGetFrameworkWrapper.Reflection.cs is the opt-out path.
internal sealed class NuGetFrameworkWrapperVendored : INuGetFrameworkWrapper
{
    ...
    public string GetTargetFrameworkVersion(string tfm, int minVersionPartCount) =>
        NuGetFrameworkWrapper.GetNonZeroVersionParts(Parse(tfm).Version, minVersionPartCount);
    ...
    public string GetTargetPlatformVersion(string tfm, int minVersionPartCount) =>
        NuGetFrameworkWrapper.GetNonZeroVersionParts(Parse(tfm).PlatformVersion, minVersionPartCount);
    ...
    public string FilterTargetFrameworks(string incoming, string filter) =>
        NuGetFrameworkWrapper.FilterTargetFrameworks<NuGetFramework, NuGetFrameworkAdapter>(incoming, filter, default);

    private readonly struct NuGetFrameworkAdapter : NuGetFrameworkWrapper.ITfmAdapter<NuGetFramework>
    {
