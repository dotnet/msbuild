// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.

using System;
using System.IO;
using System.Security;
using Microsoft.Build.Evaluation;
using Microsoft.Build.Framework;
using Microsoft.Build.UnitTests;

using Shouldly;

using Xunit;

namespace Microsoft.Build.Engine.UnitTests
{
    /// <summary>
    /// Tests for the <c>VerifyBootstrapRuntimeVersion</c> target in <c>eng/BootStrapMsBuild.targets</c>,
    /// which errors when the acquired bootstrap runtime is older than the runtime the building SDK ships.
    /// </summary>
    public sealed class BootstrapRuntimeVersion_Tests
    {
        private readonly ITestOutputHelper _output;

        public BootstrapRuntimeVersion_Tests(ITestOutputHelper output) => _output = output;

        [Theory]
        [InlineData("10.0.8", "10.0.8")]   // Exactly the SDK runtime is acceptable.
        [InlineData("10.0.8", "10.0.9")]   // A newer patch satisfies the requirement.
        [InlineData("10.0.8", "10.0.10")]  // Comparison is numeric, not lexical (10 > 8).
        [InlineData("9.0.100", "10.0.0")]  // A newer major satisfies the requirement.
        [InlineData("10.0.0-preview.7.25301.110", "10.0.0-preview.4.25258.110")] // Prerelease suffix is ignored; numeric components are equal.
        public void Passes_WhenBootstrapRuntimeAtLeastSdkRuntime(string sdkRuntime, string detectedRuntime)
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, detectedRuntime);

            bool success = RunCheck(artifactsBinDir, sdkRuntime, out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        [Fact]
        public void Fails_WhenBootstrapRuntimeOlderThanSdkRuntime()
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, "9.7.2");

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeFalse();
            BuildErrorEventArgs error = logger.Errors.ShouldHaveSingleItem();

            // "9.7.2" is not a substring of the bootstrap SDK version (10.0.300) or the required runtime
            // (10.0.8), so this proves the detected-runtime list actually renders into the message.
            string message = error.Message ?? string.Empty;
            message.ShouldContain("9.7.2");
            message.ShouldContain("10.0.8");
        }

        [Fact]
        public void Fails_WhenAllPresentRuntimesAreOlderThanSdkRuntime()
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, "9.0.5", "9.7.2");

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeFalse();
            BuildErrorEventArgs error = logger.Errors.ShouldHaveSingleItem();
            string message = error.Message ?? string.Empty;
            message.ShouldContain("9.0.5");
            message.ShouldContain("9.7.2");
        }

        [Fact]
        public void Passes_WhenAnyPresentRuntimeSatisfiesSdkRuntime()
        {
            // The host rolls forward to the highest compatible runtime, so one satisfying runtime is enough.
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, "10.0.3", "10.0.9");

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        [Fact]
        public void Passes_WhenNoBootstrapRuntimeIsPresent()
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = env.CreateFolder().Path;

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        [Fact]
        public void Passes_WhenSdkRuntimeVersionUnknown()
        {
            // When BundledNETCoreAppPackageVersion is unset there is nothing to compare against, so the
            // check must no-op even when an older runtime is present.
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, "9.0.0");

            bool success = RunCheck(artifactsBinDir, sdkRuntime: string.Empty, out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        [Fact]
        public void Passes_WhenSharedRuntimeRootIsEmpty()
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env);

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        [Fact]
        public void Passes_WhenOnlyNonVersionDirectoriesPresent()
        {
            // A stray non-version directory must be ignored, not fed into the version comparison.
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, "host");

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        [Fact]
        public void Passes_WhenPartiallyNumericDirectoryPresent()
        {
            // A name like "10.0.bad" starts with digits but is not a parseable version. It must be
            // filtered out rather than reach the comparison, which would otherwise throw.
            using TestEnvironment env = TestEnvironment.Create(_output);
            string artifactsBinDir = CreateBootstrapWithRuntimes(env, "10.0.bad");

            bool success = RunCheck(artifactsBinDir, "10.0.8", out MockLogger logger);

            success.ShouldBeTrue();
            logger.Errors.ShouldBeEmpty();
        }

        private static string CreateBootstrapWithRuntimes(TestEnvironment env, params string[] runtimeVersions)
        {
            string artifactsBinDir = env.CreateFolder().Path;
            string sharedRoot = Path.Combine(artifactsBinDir, "bootstrap", "core", "shared", "Microsoft.NETCore.App");

            // Always create the shared-framework root so callers can exercise the "root exists but is empty" path.
            Directory.CreateDirectory(sharedRoot);
            foreach (string version in runtimeVersions)
            {
                Directory.CreateDirectory(Path.Combine(sharedRoot, version));
            }

            return artifactsBinDir;
        }

        private bool RunCheck(string artifactsBinDir, string sdkRuntime, out MockLogger logger)
        {
            string targetsFile = LocateRepositoryEngFile("BootStrapMsBuild.targets");

            string projectXml = $"""
                <Project>
                  <PropertyGroup>
                    <TargetFramework>net10.0</TargetFramework>
                    <BootstrapSdkVersion>10.0.300</BootstrapSdkVersion>
                    <ArtifactsBinDir>{SecurityElement.Escape(artifactsBinDir)}{Path.DirectorySeparatorChar}</ArtifactsBinDir>
                    <BundledNETCoreAppPackageVersion>{SecurityElement.Escape(sdkRuntime)}</BundledNETCoreAppPackageVersion>
                  </PropertyGroup>
                  <Import Project="{SecurityElement.Escape(targetsFile)}" />
                </Project>
                """;

            using ProjectCollection collection = new();
            using ProjectFromString projectFromString = new(projectXml, null, null, collection);
            logger = new MockLogger(_output);

            // Building the target by name does not trigger an SDK download: it is hooked via
            // AfterTargets="AcquireSdk" rather than depended upon.
            return projectFromString.Project.Build("VerifyBootstrapRuntimeVersion", [logger]);
        }

        private static string LocateRepositoryEngFile(string fileName)
        {
            for (string? dir = AppContext.BaseDirectory; dir is not null; dir = Path.GetDirectoryName(dir))
            {
                string candidate = Path.Combine(dir, "eng", fileName);
                if (File.Exists(candidate))
                {
                    return candidate;
                }
            }

            throw new FileNotFoundException($"Could not locate eng/{fileName} by walking up from '{AppContext.BaseDirectory}'.");
        }
    }
}
