    ...
    public class ProjectInstance_Internal_Tests
    {
        ...
        public void GetTaskRegistrations()
        {
            try
            {
                ...
                project.TaskRegistry.TaskRegistrations[new TaskRegistry.RegisteredTaskIdentity("t0", new TaskHostParameters())][0].TaskFactoryAssemblyLoadInfo.AssemblyFile.ShouldBe(Path.Combine(Directory.GetCurrentDirectory(), "af0"));
                project.TaskRegistry.TaskRegistrations[new TaskRegistry.RegisteredTaskIdentity("t1", new TaskHostParameters())][0].TaskFactoryAssemblyLoadInfo.AssemblyFile.ShouldBe(Path.Combine(Directory.GetCurrentDirectory(), "af1a"));
                project.TaskRegistry.TaskRegistrations[new TaskRegistry.RegisteredTaskIdentity("t1", new TaskHostParameters())][1].TaskFactoryAssemblyLoadInfo.AssemblyName.ShouldBe("an1");
                project.TaskRegistry.TaskRegistrations[new TaskRegistry.RegisteredTaskIdentity("t2", new TaskHostParameters())][0].TaskFactoryAssemblyLoadInfo.AssemblyName.ShouldBe("an2");
            }
            finally
            {
