...
public sealed class ProjectInstanceSnapshot_Tests
{
    ...
    [Fact]
    public void Create_RejectsRecordedEvaluatedItemElements()
    {
        using var collection = new ProjectCollection();
        using var projectFromString = new ProjectRootElementFromString(ProjectContents, collection);
        ProjectInstance source = ProjectInstance.FromProjectRootElement(
        ...
        source.EvaluatedItemElements.Count.ShouldBe(2);

        Should.Throw<InvalidOperationException>(() => ProjectInstanceSnapshot.Create(source));
    }
