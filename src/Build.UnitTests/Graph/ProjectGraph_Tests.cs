...
"
        ...
        public void SyntheticSolutionNodeIsIncludedInTopologicalSort()
        {
            TransientTestFile projectFile = CreateProjectFile(
                env: _env,
                projectNumber: 1,
                extraContent: "<PropertyGroup><ProjectOnlyProperty>ProjectOnlyValue</ProjectOnlyProperty></PropertyGroup>");
            ...
            syntheticSolutionNode.ProjectInstance.GetPropertyValue("MSBuildProjectFullPath").ShouldBe(solutionFile.Path);
            syntheticSolutionNode.ProjectInstance.GetProperty("ProjectOnlyProperty").ShouldBeNull();
            syntheticSolutionNode.ProjectType.ShouldBe(ProjectInterpretation.ProjectType.NonMultitargeting);
            syntheticSolutionNode.ProjectInstance.Targets.ShouldContainKey("Build");
            projectCollection.ProjectRootElementCache.TryGet(solutionFile.Path).ShouldBeNull();
        }
        ...
        public void BuildApiUsesGeneratedSolutionTraversalInstance()
        {
            ...
            solutionNode.ProjectInstance.GetItems(ItemTypeNames.ProjectReferenceTargets)
                .ShouldContain(item =>
        ...
        public void SyntheticSolutionNodeCanBeDisabledByChangeWave()
        {
            try
            {
                ...
                ProjectGraphNode projectNode = graph.ProjectNodes.ShouldHaveSingleItem();
                graph.EntryPointNodes.ShouldHaveSingleItem().ShouldBe(projectNode);
                graph.GraphRoots.ShouldHaveSingleItem().ShouldBe(projectNode);
                projectNode.ProjectInstance.FullPath.ShouldBe(projectFile.Path);
                projectNode.ProjectInstance.GlobalProperties
                    .ContainsKey(SolutionProjectGenerator.SolutionGraphBuildEntryPointProperty)
                    .ShouldBeFalse();
                graph.TestOnly_Edges.Count.ShouldBe(0);
            }
            finally
            {
                ChangeWaves.ResetStateForTests();
