...
namespace Microsoft.Build.UnitTests.BackEnd
{
    ...
    public class TargetBuilder_Tests : IRequestBuilderCallback, IDisposable
    {
        ...
        /// <summary>
        /// Optional handler for <see cref="IRequestBuilderCallback.BlockOnTargetInProgress"/>, used by tests which
        /// need to simulate another request building targets for the same configuration.
        /// </summary>
        private Func<int, string, BuildResult, Task> _blockOnTargetInProgress;
...
";
        ...
        public void SkipNonexistentTargetsDoesNotExecuteOrCacheTargetResult()
        {
            ...
            BuildRequestEntry entry = new BuildRequestEntry(CreateNewBuildRequest(1, target, BuildRequestDataFlags.SkipNonexistentTargets), cache[1], CreateStubTaskEnvironment());
            var buildResult = builder.BuildTargets(GetProjectLoggingContext(entry), entry, this, target, CreateStandardLookup(project), CancellationToken.None).Result;
        ...
        /// </summary>
        [Fact]
        public void SkippedTargetDoesNotOverwriteResultFromReentrantRequest()
        {
            string projectContents = @"
...
";
  ...
  <Target Name='BeforeVictim' BeforeTargets='Victim' />";
        ...
        /// <summary>
        /// Not implemented, unless a test opted in via <see cref="_blockOnTargetInProgress"/>.
        /// </summary>
        ...
        {
            if (_blockOnTargetInProgress == null)
            {
                throw new NotImplementedException();
            }

            return _blockOnTargetInProgress(blockingRequestId, blockingTarget, partialBuildResult);
        }
        ...
        private BuildResult BuildSimpleProject(string projectBody, (string name, TargetBuiltReason reason)[] targets, int failTaskNumber)
        {
            ...
            TargetBuilder builder = (TargetBuilder)_host.GetComponent(BuildComponentType.TargetBuilder);
            IConfigCache cache = (IConfigCache)_host.GetComponent(BuildComponentType.ConfigCache);
            BuildRequestEntry entry = new BuildRequestEntry(CreateNewBuildRequest(1, targets), cache[1], CreateStubTaskEnvironment());
