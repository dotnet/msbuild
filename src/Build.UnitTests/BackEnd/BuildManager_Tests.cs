...
"));
        ...
        public void GraphBuildSolutionIncludesSyntheticSolutionNodeInResults()
        {
            ...
            // Verify that both the project and solution hooks ran.
        ...
        public void GraphBuildGeneratedSolutionProjectUsesInMemoryInstance()
        {
            ...
            GraphBuildRequestData request = new(
                solutionFile.Path,
                new Dictionary<string, string>(),
                ["Build"],
            ...
            ProjectGraphNode solutionNode = result.ResultsByNode.Keys.Single(
                node => node.ProjectInstance.GlobalProperties.ContainsKey(SolutionProjectGenerator.SolutionGraphBuildEntryPointProperty));
        ...
        [Fact]
        public void GraphBuildRequestSolutionImportedTargetRunsOnlyOnSolutionNode()
        {
            ...
            _env.CreateFile(
                ...
                <Project>
                  <Target Name="Build">
                    <Message Text="ProjectBuildShouldNotRun" Importance="High" />
                  </Target>
                </Project>
                """);
            TransientTestFile solutionFile = CreateSolution(
            ...
            _env.CreateFile(
                ...
                <Project>
                  <Target Name="SolutionOnly">
                    <Message Text="SolutionOnlyTargetRan" Importance="High" />
                  </Target>
                </Project>
                """);
            ...
            _logger.AssertLogContains("SolutionOnlyTargetRan");
            _logger.AssertLogDoesntContain("ProjectBuildShouldNotRun");
            _logger.AssertLogDoesntContain("MSB4057");
        }
        ...
        [Fact]
        public void GraphBuildSolutionHookCanBuildGeneratedMetaprojectReferences()
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            ProjectCollection projectCollection = env.CreateProjectCollection().Collection;
            ...
            env.CreateFile(project1Folder, "Project1.csproj", "<Project><Target Name=\"Build\" /></Project>");
            env.CreateFile(project2Folder, "Project2.csproj", "<Project><Target Name=\"Build\" /></Project>");

            TransientTestFile solutionFile = env.CreateFile(root, "Dependencies.sln",
                """
                Microsoft Visual Studio Solution File, Format Version 12.00
                # Visual Studio Version 16
                VisualStudioVersion = 16.0.29326.124
            ...
            env.CreateFile(root, $"after.{Path.GetFileName(solutionFile.Path)}.targets",
                """
                <Project>
                  <Target Name="BuildSolutionReferences" AfterTargets="Build">
                    <MSBuild
                    ...
                    <Message Text="SolutionHookBuiltReferences" Importance="High" />
                  </Target>
                </Project>
                """);
            ...
            solutionNode.ProjectInstance.GetItems("ProjectReference")
                .ShouldContain(item => item.EvaluatedInclude.EndsWith(".metaproj", StringComparison.OrdinalIgnoreCase));
            _logger.AssertLogContains("SolutionHookBuiltReferences");
        }
        ...
        [Fact]
        public void GraphBuildProjectFailureDoesNotExecuteSolutionHooks()
        {
            using TestEnvironment env = TestEnvironment.Create(_output);
            ProjectCollection projectCollection = env.CreateProjectCollection().Collection;
            ...
            env.CreateFile(projectFolder, "Project.csproj",
                ...
                <Project>
                  <Target Name="Build">
                    <Error Text="Expected project failure" />
                  </Target>
                </Project>
                """);
            ...
            env.CreateFile(root, $"after.{Path.GetFileName(solutionFile.Path)}.targets",
                $$"""
                <Project>
                  <Target Name="SolutionHook" BeforeTargets="Build">
                    <WriteLinesToFile File="{{solutionHookOutput}}" Lines="SolutionHookRan" />
                  </Target>
                </Project>
                """);
            ...
            result.OverallResult.ShouldBe(BuildResultCode.Failure);
            File.Exists(solutionHookOutput).ShouldBeFalse();
            _logger.AssertLogContains("Expected project failure");
        }
        ...
        public void GraphBuildSolutionChangeWaveOptOutPreservesLegacyExecution()
        {
            try
            {
                ...
                GraphBuildRequestData request = new(
                    new ProjectGraphEntryPoint(solutionFile.Path),
                    Array.Empty<string>(),
                    projectCollection.HostServices);
                ...
                result.OverallResult.ShouldBe(BuildResultCode.Success);
                ProjectGraphNode projectNode = result.ResultsByNode.Keys.ShouldHaveSingleItem();
                projectNode.ProjectInstance.FullPath.ShouldBe(Path.Combine(projectFolder.Path, "Project.csproj"));
