...
internal sealed partial class CoordinatorServer(CoordinatorSettings settings, ICoordinatorDebugOutput? output = null) : IDisposable
{
    ...
    private readonly NodeBudgetManager _budgetManager = new(
        ...
        settings.MaxNodesPerBuildWhenIdle);
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_settings.DefaultNodeSettingsOptOutMessage is { } defaultNodeSettingsOptOutMessage)
        {
            _output.WriteLine($"CoordinatorServer: Default node settings are active. {defaultNodeSettingsOptOutMessage}");
        }
    ...
    private static string FormatMaxNodesPerBuildWhenIdle(int value)
        => value == 0 ? "0 (disabled)" : value.ToString();
