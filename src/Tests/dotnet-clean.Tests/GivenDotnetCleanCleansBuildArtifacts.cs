...
namespace Microsoft.DotNet.Cli.Clean.Tests
{
    public class GivenDotnetCleanCleansBuildArtifacts : SdkTest
    {
        ...
        public void ItCleansAProjectBuiltWithRuntimeIdentifier()
        {
            ...
            var outputFolder = new DirectoryInfo(OutputPathCalculator.FromTestAsset(testInstance.Path).GetOutputDirectory(configuration: configuration, runtimeIdentifier: $"{ToolsetInfo.LatestWinRuntimeIdentifier}-x64"));
