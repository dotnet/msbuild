...
namespace Microsoft.DotNet.Cli.CheckUpdate.Tests
{
    public class GivenDotnetCheckUpdate : SdkTest
    {
        private readonly BufferedReporter _reporter;
        private readonly string fakeReleasesPath;

        private const string HelpText = @"check-update:
      .NET Check Update Command
    
    Usage:
      dotnet check-update [options]
    
    Options:
      -?, -h, --help    Show help and usage information";

        public GivenDotnetCheckUpdate(ITestOutputHelper log) : base(log)
        {
        ...
        {
            var parseResult = Parser.Instance.Parse(new string[] { "dotnet", "check-update" });
            ...
            new CheckUpdateCommand(parseResult, new MockNETBundleProvider(newerBandExists ? oldSdks : newSdks), new MockProductCollectionProvider(fakeReleasesPath), _reporter).Execute();
        ...
        public void ItContainsInfoForAllInstalledBundles()
        {
            ...
            new CheckUpdateCommand(parseResult, new MockNETBundleProvider(bundles), new MockProductCollectionProvider(fakeReleasesPath), _reporter).Execute();
