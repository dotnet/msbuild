...
namespace Microsoft.Build.UnitTests
{
    public partial class TestEnvironment : IDisposable
    {
        ...
        /// <summary>
        ///     Files registered via <see cref="PublishTestFile(string, bool)"/> to be collected as CI
        ///     build artifacts when the test completes.
        /// </summary>
        private readonly List<(string Path, bool Always)> _filesToPublish = new List<(string, bool)>();
        ...
        private void Cleanup()
        {
            if (!_disposed)
            {
                ...
                // Publish registered files (e.g. binary logs) before reverting variants, which would
                // otherwise delete the transient files/folders they live in.
                PublishRegisteredFiles();
        ...
        /// <summary>
        ///     Registers a file to be collected as a CI build artifact when the test completes.
        /// </summary>
        /// <remarks>
        ///     <para>
        ///     This is intended for diagnostics that are produced out-of-band &#8212; most notably binary logs
        ///     (<c>-bl</c>) emitted by out-of-process MSBuild invocations &#8212; which would otherwise be lost when
        ///     test cleanup deletes the transient files and folders they live in.
        ///     </para>
        ///     <para>
        ///     On cleanup the file is copied into the build's log output directory
        ///     (<c>artifacts/log/&lt;Configuration&gt;/TestArtifacts</c>). That directory is published as a build
        ///     artifact by CI, so registered files are collected automatically without any pipeline changes. The
        ///     file path is also written to the test output, and the file is attached to the xUnit test result on a
        ///     best-effort basis.
        ///     </para>
        ///     <para>
        ///     Typical usage is to point a binary log at a test-environment-unique path, run the build, and register
        ///     that path:
        ///     <code>
        ///     string binlog = _env.ExpectFile(".binlog").Path;
        ///     _env.PublishTestFile(binlog);
        ///     RunnerUtilities.ExecBootstrapedMSBuild($"{project} -bl:{binlog}", out bool success);
        ///     </code>
        ///     </para>
        /// </remarks>
        /// <param name="filePath">The absolute path of the file to publish.</param>
        /// <param name="always">
        ///     When <see langword="false"/> (default), the file is only published if the test did not pass, keeping
        ///     artifacts from passing runs to a minimum. When <see langword="true"/>, the file is published
        ///     regardless of the test outcome.
        /// </param>
        public void PublishTestFile(string filePath, bool always = false)
        {
            ...
            _filesToPublish.Add((filePath, always));
        }

        /// <summary>
        ///     Copies and/or attaches files registered via <see cref="PublishTestFile(string, bool)"/> so they are
        ///     collected as CI build artifacts. Invoked during cleanup before transient state is reverted.
        /// </summary>
        private void PublishRegisteredFiles()
        {
            if (_filesToPublish.Count == 0)
            {
                return;
            }

            bool testPassed = DidCurrentTestPass();
            string destinationDirectory = ResolvePublishDirectory();
            string testNamePrefix = GetCurrentTestNamePrefix();

            foreach ((string path, bool always) in _filesToPublish)
            {
                try
                {
                    if (!always && testPassed)
                    {
                        continue;
                    }
                    ...
                    if (!File.Exists(path))
                    {
                        Output?.WriteLine($"[PublishTestFile] Skipping '{path}' because it does not exist.");
                        continue;
                    }

                    if (string.IsNullOrEmpty(destinationDirectory))
                    {
                        Output?.WriteLine($"[PublishTestFile] No artifacts log directory is available; '{path}' was not published.");
                    }
                    else
                    {
                        Directory.CreateDirectory(destinationDirectory);
                        string destination = GetUniqueDestinationPath(destinationDirectory, testNamePrefix + Path.GetFileName(path));
                        File.Copy(path, destination, overwrite: true);
                        Output?.WriteLine($"[PublishTestFile] Published '{path}' to '{destination}'.");
                    }

                    TryAttachToTestResult(path);
                }
                catch (Exception e)
                {
                    Output?.WriteLine($"[PublishTestFile] Failed to publish '{path}': {e}");
                }
            }
        }

        /// <summary>
        ///     The directory into which published files are copied. This lives under the build's log output
        ///     directory, which CI publishes as a build artifact. Returns <see langword="null"/> if the location is
        ///     unknown (for example, when running outside the build).
        /// </summary>
        private static string ResolvePublishDirectory()
        {
            string logDirectory = RunnerUtilities.ArtifactsLocationAttribute?.ArtifactsLogLocation;
            return string.IsNullOrEmpty(logDirectory) ? null : Path.Combine(logDirectory, "TestArtifacts");
        }

        /// <summary>
        ///     Returns <see langword="true"/> only when the current test is known to have passed. Any other state
        ///     (failed, skipped, unknown, or not running under xUnit) returns <see langword="false"/> so that
        ///     diagnostics are preserved by default.
        /// </summary>
        private static bool DidCurrentTestPass()
        {
            try
            {
                return Xunit.TestContext.Current?.TestState?.Result == Xunit.TestResult.Passed;
            }
            catch
            {
                return false;
        ...
        /// <summary>
        ///     Returns a sanitized, file-name-safe prefix derived from the current test method name, or an empty
        ///     string if it cannot be determined.
        /// </summary>
        private static string GetCurrentTestNamePrefix()
        {
            try
            {
                string methodName = Xunit.TestContext.Current?.TestMethod?.MethodName;
                return string.IsNullOrEmpty(methodName) ? string.Empty : SanitizeFileName(methodName) + "_";
            }
            catch
            {
                return string.Empty;
        ...
        /// <summary>
        ///     Attaches the file to the current xUnit test result on a best-effort basis. Failures are ignored
        ///     because attachment support depends on the test pipeline stage and is purely supplementary to the
        ///     copy into the published artifacts directory.
        /// </summary>
        private static void TryAttachToTestResult(string path)
        {
            try
            {
                Xunit.TestContext.Current?.AddAttachment(Path.GetFileName(path), File.ReadAllBytes(path), "application/octet-stream");
            }
            catch
            {
                // Attachment is supplementary; ignore any failure.
            }
        }

        /// <summary>
        ///     Returns a destination path that does not collide with an existing file, disambiguating with a short
        ///     unique suffix when necessary.
        /// </summary>
