...
namespace Microsoft.Build.Shared
{
    ...
    internal static class QuotingUtilities
    {
        ...
        {
            Assumed.GreaterThanOrEqual(maxSplits, 2, "There is no point calling this method for less than two splits.");

            string separators = new StringBuilder().Append(separator).ToString();

            Assumed.Equal(separators.IndexOf('"'), -1, "The double-quote character is not supported as a separator.");
