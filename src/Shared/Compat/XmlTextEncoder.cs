...
//
        ...
        internal void WriteSurrogateChar( char lowChar, char highChar ) {
            if ( !XmlCharType.IsLowSurrogate(lowChar) ||
                ...
                throw CreateInvalidSurrogatePairException( lowChar, highChar );
            }
                    ...
                    default:
                        if ( XmlCharType.IsHighSurrogate( ch ) ) {
                            if ( i + 1 < len ) {
                                ...
                                throw CreateInvalidSurrogatePairException( text[i], ch );
                            }
                        }
                        else if ( XmlCharType.IsLowSurrogate( ch ) ) {
                            throw CreateInvalidHighSurrogateCharException( ch );
                ...
                if ( XmlCharType.IsHighSurrogate( ch ) ) {
                    if ( i + 1 < len ) {
                        ...
                        if ( XmlCharType.IsLowSurrogate( lowChar ) ) {
                            ...
                            throw CreateInvalidSurrogatePairException( lowChar, ch );
                        }
                    }
                    throw new ArgumentException( "Xml_InvalidSurrogateMissingLowChar" );
                }
                else if ( XmlCharType.IsLowSurrogate( ch ) ) {
                    throw CreateInvalidHighSurrogateCharException( ch );
                }
...
//
        ...
        //  Exception methods from XmlConvert which weren't in .NET Core
        internal static Exception CreateInvalidSurrogatePairException( char low, char hi ) {
            ...
            return new ArgumentException(string.Format("The surrogate pair (0x{0}, 0x{1}) is invalid. A high surrogate character (0xD800 - 0xDBFF) must always be paired with a low surrogate character (0xDC00 - 0xDFFF).", args));
        }

        internal static Exception CreateInvalidHighSurrogateCharException( char hi ) {
            return new ArgumentException(string.Format("Invalid high surrogate character (0x{0}). A high surrogate character must have a value from range (0xD800 - 0xDBFF).",
                ((uint)hi).ToString("X", CultureInfo.InvariantCulture)));
        }
    }
}
