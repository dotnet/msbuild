    ...
    sealed public class RequiredTransformations
    {
        ...
        public void StronglyTypedResources()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(t.FilesWritten[2].ItemSpec == stronglyTypedFileName);
        ...
        public void StronglyTypedResourcesUpToDate()
        {
            ...
            try
            {
                ...
                Utilities.AssertStateFileWasWritten(t2);
                Assert.IsTrue(t2.FilesWritten[2].ItemSpec == Path.ChangeExtension(t2.Sources[0].ItemSpec, ".cs"));
            }
            finally
            {
                // Done, so clean up.
        ...
        public void StronglyTypedResourcesOutOfDate()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(t.FilesWritten[2].ItemSpec == strFile);
...
namespace Microsoft.Build.UnitTests.GenerateResource_Tests
{
    ...
    internal sealed partial class Utilities
    {
        ...
        {
            Assert.IsTrue(((MockEngine)t.BuildEngine).Log.Contains(message));
        }
        ...
        {
            Assert.IsNotNull(t.FilesWritten, "The state file should have been written, but there aren't any.");
            Assert.IsNotNull(t.StateFile, "State file should be defined");
            Assert.IsTrue(File.Exists(t.StateFile.ItemSpec), "State file should exist");
            ...
            Assert.IsTrue(foundStateFile, "Expected there to be a state file, but there wasn't");
        }
        ...
        public static void ExecuteTask(GenerateResource t)
        {
            ...
            if (t.OutputResources != null && t.OutputResources[0] != null)
            {
                ...
                if (t.Sources[0] != null)
                    Assert.IsTrue(resourcesLastModified >= File.GetLastWriteTime(t.Sources[0].ItemSpec));
            }
        }
        ...
        public static void STRNamespaceTestHelper(string strLanguage, string resourcesNamespace, string classNamespace)
        {
            ...
            Assert.IsNotNull(strLanguage);
            Assert.IsNotNull(resourcesNamespace);
            // Generate Task
            GenerateResource t = Utilities.CreateTask();
            try
            {
                ...
                Assert.IsTrue(Path.GetFileNameWithoutExtension(textFile).Equals(Path.GetFileNameWithoutExtension(t.OutputResources[0].ItemSpec)));
                ...
                // Files written should contain STR class file            
                Assert.AreEqual(Path.ChangeExtension(t.Sources[0].ItemSpec, codeFileExtension), t.StronglyTypedFileName);
                Assert.IsTrue(t.FilesWritten[2].ItemSpec == t.StronglyTypedFileName);
                ...
                Assert.IsTrue(Utilities.ReadFileContent(STRFile).ToLower().Contains("class " + Path.GetFileNameWithoutExtension(textFile).ToLower()));
                // Verify that the namespace of the STR class is as expected 

                Assert.IsTrue(!Utilities.ReadFileContent(STRFile).ToLower().Contains("namespace " + resourcesNamespace.ToLower()));
                if (classNamespace != null)
                    Assert.IsTrue(Utilities.ReadFileContent(STRFile).ToLower().Contains("namespace " + classNamespace.ToLower()));
