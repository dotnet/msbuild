...
namespace Microsoft.NET.Build.Tasks
{
    internal class RuntimeGraphCache
    {
        ...
        public RuntimeGraph GetRuntimeGraph(string runtimeJsonPath)
        {
            ...
            RuntimeGraph result;
            object existingRuntimeGraphTaskObject = _buildEngine.GetRegisteredTaskObject(key, RegisteredTaskObjectLifetime.Build);
            if (existingRuntimeGraphTaskObject == null)
            {
                result = JsonRuntimeFormat.ReadRuntimeGraph(runtimeJsonPath);

                _buildEngine.RegisterTaskObject(key, result, RegisteredTaskObjectLifetime.Build, true);
            }
            else
            {
