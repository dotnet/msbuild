...
namespace Microsoft.Build.UnitTests.ResolveAssemblyReference_Tests
{
    public class ResolveAssemblyReferenceTestFixture : IDisposable
    {
        ...
        internal static bool FileExists(string path)
        {
            ...
            if (path.Contains("MyMissingAssembly", StringComparison.CurrentCulture))
            {
                return false;
            }
        ...
        {
            if (path.EndsWith(s_myVersion20Path, StringComparison.CurrentCulture))
            {
        ...
        internal static string GetRuntimeVersion(string path)
        {
            ...
            else if (path.StartsWith(@"C:\WinMDArchVerification", StringComparison.OrdinalIgnoreCase) && path.EndsWith(".winmd", StringComparison.CurrentCulture))
            {
                return "WindowsRuntime 1.0";
            }
            else if (path.EndsWith(".dll", StringComparison.CurrentCulture) || path.EndsWith(".exe", StringComparison.CurrentCulture) || path.EndsWith(".winmd", StringComparison.CurrentCulture))
            {
                return "v2.0.50727";
            }
            else
        ...
        internal static AssemblyNameExtension GetAssemblyName(string path)
        {
            ...
            if (path.EndsWith(Path.Combine(s_myVersion20Path, "MyGacAssembly.dll"), StringComparison.CurrentCulture))
            {
                // Simulate a strongly named assembly.
            ...
            if (path.EndsWith(s_myLibraries_V1_DDllPath, StringComparison.CurrentCulture))
            {
                // Version 1 of D
                return new AssemblyNameExtension("D, Version=1.0.0.0, CulTUre=neutral, PublicKeyToken=aaaaaaaaaaaaaaaa");
            }

            if (path.EndsWith(@"c:\RogueLibraries\v1\D.dll", StringComparison.CurrentCulture))
            {
            ...
            if (path.EndsWith(s_myLibraries_V1_E_EDllPath, StringComparison.CurrentCulture))
            {
            ...
            if (path.EndsWith(s_myLibraries_V2_DDllPath, StringComparison.CurrentCulture))
            {
                return new AssemblyNameExtension("D, VErsion=2.0.0.0, CulturE=neutral, PublicKEyToken=aaaaaaaaaaaaaaaa");
            }

            if (path.EndsWith(s_myLibraries_V1_GDllPath, StringComparison.CurrentCulture))
            {
                return new AssemblyNameExtension("G, Version=1.0.0.0, Culture=neutral, PublicKEyToken=aaaaaaaaaaaaaaaa");
            }

            if (path.EndsWith(s_myLibraries_V2_GDllPath, StringComparison.CurrentCulture))
            {
