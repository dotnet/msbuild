...
namespace Microsoft.Build.UnitTests
{
    ...
    public class WriteCodeFragment_Tests
    {
        ...
        /// </summary>
        [Fact]
        public void CommentIsInvariantCulture()
        {
            ...
            try
            {
                ...
                WriteCodeFragment taskEnglish = new WriteCodeFragment();
                MockEngine engineEnglish = new MockEngine(true);
                taskEnglish.BuildEngine = engineEnglish;
                TaskItem attributeEnglish = new TaskItem("System.AssemblyVersionAttribute");
                attributeEnglish.SetMetadata("_Parameter1", "1.0.0.0");
                taskEnglish.AssemblyAttributes = [attributeEnglish];
                taskEnglish.Language = "c#";
                taskEnglish.OutputDirectory = new TaskItem(Path.GetTempPath());
                bool resultEnglish = taskEnglish.Execute();
                
                resultEnglish.ShouldBeTrue();
                string contentEnglish = File.ReadAllText(taskEnglish.OutputFile.ItemSpec);
                
                // Test with a different culture (French)
                System.Globalization.CultureInfo.CurrentUICulture = new System.Globalization.CultureInfo("fr-FR");
                
                WriteCodeFragment taskFrench = new WriteCodeFragment();
                MockEngine engineFrench = new MockEngine(true);
                taskFrench.BuildEngine = engineFrench;
                TaskItem attributeFrench = new TaskItem("System.AssemblyVersionAttribute");
                attributeFrench.SetMetadata("_Parameter1", "1.0.0.0");
                taskFrench.AssemblyAttributes = [attributeFrench];
                taskFrench.Language = "c#";
                taskFrench.OutputDirectory = new TaskItem(Path.GetTempPath());
                bool resultFrench = taskFrench.Execute();
                ...
                commentFrench.ShouldNotBeNullOrWhiteSpace();
                commentEnglish.ShouldBe(commentFrench);
                
                // Verify it's the expected invariant English text
                commentEnglish.Trim().ShouldBe("// Generated by the MSBuild WriteCodeFragment class.");
                
                // Cleanup
                if (taskEnglish.OutputFile != null && File.Exists(taskEnglish.OutputFile.ItemSpec))
                {
                    File.Delete(taskEnglish.OutputFile.ItemSpec);
                }
                if (taskFrench.OutputFile != null && File.Exists(taskFrench.OutputFile.ItemSpec))
                {
                    File.Delete(taskFrench.OutputFile.ItemSpec);
                }
            }
            finally
            {
