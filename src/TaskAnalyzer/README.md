...
Both the MSBuildTask0002 and MSBuildTask0003 fixers reference the instance `TaskEnvironment` member, so no fix is offered when the flagged call sits in a **static** context (a static method, static local function, or static lambda) — emitting one there would not compile. Making the enclosing member non-static, or threading a `TaskEnvironment`/`AbsolutePath` parameter through it, re-enables the fix.
