...
function Write-RegressionDetectionReport
{
    ...
    [void]$markdown.AppendLine()
    [void]$markdown.AppendLine("These are statistical candidates, not confirmed product regressions. The detector compares the latest production MT/non-MT pair with paired production runs from the preceding $($detector.lookbackDays) days. A candidate must:")
    [void]$markdown.AppendLine()
    [void]$markdown.AppendLine("- regress by at least $($detector.minimumMtRegressionPercent)% and $($detector.minimumMtRegressionMs) ms against the historical MT median;")
    [void]$markdown.AppendLine('- exceed the historical MT p90;')
    [void]$markdown.AppendLine("- have at least $($detector.minimumBaselineRuns) paired baseline runs;")
    [void]$markdown.AppendLine("- show at least $($detector.minimumMtRegressionMs) ms of deterioration in the MT-minus-non-MT differential; and")
    [void]$markdown.AppendLine("- use a current paired run no older than $($detector.freshnessDays) days.")
    [void]$markdown.AppendLine()
