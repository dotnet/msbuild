...
description: >-
  ...
  NOT rebuild: it inspects the PR's **latest** Azure Pipelines `msbuild-pr`
...
# DevOps `msbuild-pr` build instead of rebuilding. Mirrors the fetch-binlog job
    ...
    steps:
      ...
      - name: Download binlogs from the PR's latest failed Azure Pipelines build
        ...
        env:
          ...
          # msbuild-pr pipeline definition id in dnceng-public/public.
        ...
        run: |
          ...
          # --- Scope check: only analyse PRs targeting main / vs* ----------
          ...
          # --- Find the PR's most recent msbuild-pr build (merge ref) --------
          ...
          echo "Newest msbuild-pr build for PR #${PR_NUMBER}: id='${BUILD_ID}' status='${BUILD_STATUS}' result='${BUILD_RESULT}'"
          [ -z "${BUILD_ID}" ] && { echo "::warning::No msbuild-pr build found for PR #${PR_NUMBER}."; emit_none; }
          ...
          if [ "${BUILD_STATUS}" != "completed" ]; then
            echo "::warning::PR #${PR_NUMBER}'s newest msbuild-pr build (${BUILD_ID}) is still '${BUILD_STATUS}'; wait for it to finish before analysing."
            emit_none
          fi
          if [ "${BUILD_RESULT}" != "failed" ]; then
            echo "::warning::PR #${PR_NUMBER}'s newest msbuild-pr build (${BUILD_ID}) result is '${BUILD_RESULT}', not failed — the failure looks resolved; nothing to analyse."
            emit_none
          fi
