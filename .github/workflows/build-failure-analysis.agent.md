...
description: >-
  When the Azure Pipelines PR build (`msbuild-pr`) fails, downloads the binary
...
# own**. MSBuild's authoritative PR build runs on Azure DevOps
# (dnceng-public/public, pipeline "msbuild-pr", definitionId 75) and publishes
...
on:
  ...
  # below filters tightly to the `msbuild-pr` build check reporting failure.
  check_run:
    types: [completed]
...
# skipped agent — no AI calls on anything but a real `msbuild-pr` failure whose
...
concurrency:
  # Only real `msbuild-pr` check_run events (and manual dispatch for a PR) use a
  ...
  group: ${{ (github.event_name == 'check_run' && github.event.check_run.name == 'msbuild-pr' && format('build-failure-analysis-{0}', github.event.check_run.pull_requests[0].number || github.event.check_run.head_sha)) || (github.event_name == 'workflow_dispatch' && format('build-failure-analysis-{0}', inputs['pr-number'])) || format('build-failure-analysis-run-{0}', github.run_id) }}
...
jobs:
  fetch-binlog:
    ...
    timeout-minutes: 15
    # `check_run` fires for every check; only act on the MSBuild PR build check
    # reporting failure (or a manual dispatch).
    if: >
      github.event_name == 'workflow_dispatch' ||
      (github.event.check_run.name == 'msbuild-pr' && github.event.check_run.conclusion == 'failure')
    permissions:
      contents: read
      pull-requests: read
    outputs:
    ...
    steps:
      - name: Download binlogs from the failed Azure Pipelines build
        ...
        env:
          ...
          # msbuild-pr pipeline definition id in dnceng-public/public (used to
          # validate a dispatched build id belongs to the right pipeline).
          ADO_BUILD_DEFINITION_ID: "75"
        ...
        run: |
          ...
          # --- 3. Scope check: only analyse PRs targeting main / vs* --------
          ...
          case "${BASE_REF}" in
            main|vs*) echo "PR #${PR_NUMBER} base '${BASE_REF}' is in scope." ;;
            *) echo "::warning::PR #${PR_NUMBER} base '${BASE_REF}' is out of scope (main, vs*); skipping."; emit_none ;;
          esac

          # --- 4. Validate the build for EVERY trigger (not just dispatch):
          #        it must be the msbuild-pr definition (75), have failed, and
          ...
          if [ "${DEF_ID}" != "${ADO_BUILD_DEFINITION_ID}" ]; then
            echo "::warning::ADO build ${BUILD_ID} is definition '${DEF_ID}', not msbuild-pr (${ADO_BUILD_DEFINITION_ID}); refusing."; emit_none
          fi
          ...
          # MSBuild publishes separate build/test log artifacts per leg. Map
          # only failed/canceled timeline jobs to their build-log artifacts;
          # each selected archive can contain both Build.binlog (bootstrap)
          # and Build.stage2.binlog.
          ...
          mapfile -t all_names < <(
            printf '%s' "${artifacts_json}" |
              jq -r '.value // [] | map(select(.name | test(" build logs$") or test("^BuildLogs_SourceBuild_Managed_Attempt[0-9]+$"))) | .[].name'
          )
          mapfile -t names < <(
            for job_key in "${failed_job_keys[@]}"; do
              ...
              source_build=false
              case "${job_key}" in
                windowsfull) expected="FullOnWindows build logs" ;;
                windowscore) expected="CoreOnWindows build logs" ;;
                windowsfullreleasenobootstrap) expected="FullOnWindows Release build logs" ;;
                linuxcoremultithreadedmode) expected="MTModeOnLinux build logs" ;;
                linuxcore) expected="CoreOnLinux build logs" ;;
                macoscore) expected="CoreOnMac build logs" ;;
                sourcebuildmanaged) source_build=true ;;
                *) continue ;;
              esac
          ...
          echo "Selected ${#names[@]} of ${#all_names[@]} build-log artifacts for ${#failed_job_keys[@]} failed or canceled jobs."
