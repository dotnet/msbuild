param([string]$Tag = "vstest-tiny", [int]$TimeoutMinutes = 4)
...
$proc = Start-Process -FilePath cts -ArgumentList @(
    ...
    "--filter","eng/cts/vstest-sample/bin/Debug/net10.0/CtsVsTestSample.dll",
    "--dop","1",
...
if (-not $proc.WaitForExit($TimeoutMinutes * 60 * 1000)) {
    ...
    Get-Content "$sampleRoot\cts.log" -Tail 30
    exit 124
}
$sw.Stop()
Write-Host "exit=$($proc.ExitCode) duration=$([int]$sw.Elapsed.TotalSeconds)s"
Write-Host "--- stdout tail ---"
Get-Content "$sampleRoot\cts.log" -Tail 40
exit $proc.ExitCode
