...
| `LoadAssemblyAndFindTypeStart` (87)<br/>`LoadAssemblyAndFindTypeStop` (88) | No | Around loading an assembly and scanning it for candidate public types. | `assemblyPath`: assembly path inspected.<br/>`numberOfPublicTypesSearched`: number of public types scanned before completion. |
| `FallbackAssemblyLoadStart` (97)<br/>`FallbackAssemblyLoadStop` (98) | No | Around `TypeLoader`'s fallback assembly loading path. Usually only interesting when normal loading was insufficient. | `assemblyName`: assembly being loaded via the fallback path. |
