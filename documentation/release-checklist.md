...
- [ ] **5.4** Update `tools.dotnet` in [`global.json`](https://github.com/dotnet/msbuild/blob/main/global.json) to the latest released SDK in the targeted band. The bootstrap SDK is `Max(BootstrapSdkVersionFloor, tools.dotnet)`, so this normally suffices; only change `BootstrapSdkVersionFloor` in [`eng/Versions.props`](https://github.com/dotnet/msbuild/blob/main/eng/Versions.props) to pin a newer SDK, or to lower it if it now exceeds `tools.dotnet` on this branch. Check https://dotnet.microsoft.com/download/visual-studio-sdks.
